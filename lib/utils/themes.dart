import 'package:flutter/material.dart';

class Themes {
  // static const kPrimaryColor = Color(0xFFB0A9FB);
  // static const kAccentColor = Color(0xFFEFE7EE);
  // static const kLightColor = Color(0xFFF0F0F0);

  static const kPrimaryColor = Color(0xFF0E68B1);
  static const kOrangeColor = Color(0xFFFF8D0B);

  static const kLightColor = Color(0xFFF5F6FA);
  static const kBorderColor = Color(0xFFE9EAEB);
  static const kHeaderLightColor = Color(0xFFF8F8F8);
  static const kDarkColor = Color(0xFF202224);
  static const kDarkGreyColor = Color(0xFF5C5C5C);

  static const kGreenColor = Colors.green;
  static const kRedColor = Colors.red;
  static const kGreyColor = Colors.grey;
  static const kWhiteColor = Colors.white;
  static const kBlackColor = Colors.black;
  static const kTransparent = Colors.transparent;
}
